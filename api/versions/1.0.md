...
Just like our company, the APIs we build reflects need to reflect our core values. 

## Team

An API is a reflection of the team that made it. Since Flowfact works as a team, the APIs we build should be consistent and durable.

## Customer focus

The API needs to be understandle and useful, tackling real world challenges of our customers.

## Strengthen and develop strengths

The software we build exposes
...
The API needs to expose the underlying feature set of the application in a transparent manner, to make it easy to develop against.
...
The precision must be translated by clients and servers into the most specific language types.
...
**Note** When using the `Content-Location` header, the `Content-Type` header has to be set as well.
