...
Media type versioning: Here, version information and media type are provided together via the HTTP Content-Type header — e.g. `application/x.flowfact.schema.v2+json;`.
...
```
Accept: application/x.flowfact.schema.v2+json
```
...
```
Content-Type: application/x.flowfact.schema.v2+json
```
